---
title: "Calculating Nucleotide Diversity"
...
Firstly, I check if the inversion is present in the sample, but making sure to correct in cases where there are multiple mutations at that site (which theoretically shouldn't happen, but some issue in SLiM there). The rows of the MS matrix are separated depending on the presence of both inversion marker mutations, and the calculation for nucleotide diversity (including windowing) done for each separately.  
Note if the sample does not include any inverted individuals, then the nucleotide diversity for the inverted individuals of that replicate are set to NA.
...
```{r happlot, echo=FALSE, fig.width=9, fig.height=6}
